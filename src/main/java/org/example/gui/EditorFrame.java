...
import org.example.application.FileExtensions;
import org.example.application.RecentFilesQueue;
...
public class EditorFrame extends BorderPane
{
	...
	private static final String USER_MANUAL_URL = "https://www.example.org/docs/user-guide.html";
	...
	public EditorFrame(Stage pMainStage) 
	{
		...
		aRecentFiles.deserialize(Preferences.userNodeForPackage(example.class).get("recent", "").trim());
...
//		tabPane.setTabDragPolicy(TabPane.TabDragPolicy.REORDER); // This JavaFX feature is too buggy to use at the moment see issue #455
	...
	private void createHelpMenu(MenuBar pMenuBar) 
	{
		...
		pMenuBar.getMenus().add(factory.createMenu("help", false,
				...
				factory.createMenuItem("help.guide", false, event -> example.openBrowser(USER_MANUAL_URL)),
	...
	{
		String dir = Preferences.userNodeForPackage(example.class).get(pKey, ".");
	...
	{
		Preferences.userNodeForPackage(example.class).put(pKey, pLastExportDir.getAbsolutePath().toString());
	}
	...
	private void exportImage() 
	{
		FileChooser fileChooser = getImageFileChooser(getLastDir(KEY_LAST_EXPORT_DIR), 
				Preferences.userNodeForPackage(example.class).get(KEY_LAST_IMAGE_FORMAT, "png"));
		...
		Preferences.userNodeForPackage(example.class).put(KEY_LAST_IMAGE_FORMAT, format);
	...
	public void exit() 
	{
		...
		if (modcount > 0) 
		{
			...
			if (alert.getResult() == ButtonType.YES) 
			{
				Preferences.userNodeForPackage(example.class).put("recent", aRecentFiles.serialize());
				System.exit(0);
			}
		}
		else 
