...
public class ViewedTips 
{
	...
	{
		assert pFirstTipOfTheDayId >= 1 && pFirstTipOfTheDayId <= TipLoader.NUM_TIPS;
		
		aCurrentTipId = pFirstTipOfTheDayId;
		
		if (pFirstTipOfTheDayId == TipLoader.NUM_TIPS)
		{
	...
	{
		if (aCurrentTipId == TipLoader.NUM_TIPS)
		{
			aCurrentTipId = 1;
		}
		else
	...
	private void updateCurrentTipIdToPreviousTipId()
	{
		if (aCurrentTipId == 1)
		{
			aCurrentTipId = TipLoader.NUM_TIPS;
		}
		else
		{
			aCurrentTipId = aCurrentTipId - 1;
	...
	private void updateNextTipOfTheDayId()
	{
		...
		{
			if (aNewNextTipOfTheDayId == TipLoader.NUM_TIPS)
			{
				aNewNextTipOfTheDayId = 1;
			}
			else
